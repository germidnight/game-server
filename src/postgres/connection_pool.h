/*
 * Пул соединений — заранее подготовленный набор соединений, из которого программа их получает.
 * Перед использованием этого класса надо создать пул соединений нужного размера.
 * После конструирования пула из него можно получать свободные соединения методом GetConnection.
 * Если соединений нет, метод заблокирует текущий поток, пока не появятся свободные соединения.
 * Метод GetConnection возвращает прокси-объект ConnectionPool::ConnectionWrapper,
 * который в своём деструкторе вернёт соединение обратно в пул.
 */
#pragma once
